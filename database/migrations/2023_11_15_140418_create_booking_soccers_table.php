...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('booking_soccers', function (Blueprint $table) {
            $table->id();
            $table->string('status');
            $table->date('tanggal');

            $table->unsignedBigInteger('id_lapangan');
            $table->foreign('id_lapangan')->references('id')->on('soccers')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('booking_soccers');
    }
};